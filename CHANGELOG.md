...
- **`build_manifest` on `state-illustrator`'s output contract**, in the §Appendix A shape (`slice` / `routes` / `components[].affordances` / `serverActions` / `transitions` / `note`), plus a new Step 4 that derives it. The step calls out the affordances the agent adds itself in Step 3 (retry buttons, empty-state CTAs, dismiss controls), which are the ones most often missed because the agent that adds them has historically not been the agent that declares them, and the loading-to-populated and error-to-retry-to-populated transitions, which are what make the tester's bidirectional rule bite. `prefers-reduced-motion` verification moves to Step 5.
- **`mode: "manifest-only"` dispatch** for a `frontend` slice with zero interactive surfaces: `state-illustrator` skips the state audit and returns the manifest alone, so a static route still reaches `slice-tester` and `slice-verifier` with something to work from.
- **Zero-declared backstop rule in `slice-verifier`.** A `type: frontend` slice arriving with `manifest_declared: 0` returns `needs_human` with `hitl_category: "prd_ambiguity"`, never a silent pass. It is the one `fail` that does not route to a fixer: there is no patch for a manifest that was never emitted, so `fix_targets` stays empty and the bubble goes to a human.
...
- **Manifest ownership is now stated per slice type** across `sell-slice/SKILL.md` (routing table, Phase 4, the frontend producer pipeline, Phases 5.1/5.2/5.3, completion checklist) and mirrored into `sell-pie/SKILL.md` (Workflow A, Workflow B, agent roster, completion checklist): the `implementer` on `backend` / `full-stack` / `db-schema` / `infrastructure`, `state-illustrator` on `frontend`. On `full-stack`, where the builder already emitted the manifest, any affordance `state-illustrator` adds is merged into it before the tester runs, so the backstop does not report an under-declaration the builder never caused.
...
- **Frontend slices now emit a build manifest.** `type: frontend` routes through the internal producer pipeline and never dispatches `implementer`, which is the manifest's only emitter, so `slice-tester` was handed a manifest that was never produced and `slice-verifier`'s under-declaration backstop compared its diff-derived count against an undefined number. `state-illustrator` is now the emitter on that path: it runs last, has already read every implemented file in full, and derives the §Appendix A manifest **from the files on disk**. It does not reconcile against upstream agent returns, deliberately, since `slice-verifier` re-derives the same surface from the diff independently and a second weaker copy of that check run by the emitter is worthless. Slice 1.1 of Pie 1 in `docs/frontend-path-plan.md`.
