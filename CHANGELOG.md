...
- **Exit criteria now reach all six frontend producers.** `modern-ux-expert`, `layout-architect`, `block-composer`, `component-crafter`, `library-entry-writer`, and `state-illustrator` each gain the slice's acceptance contract as a declared input, and both delivery skills pass it verbatim in the frontend dispatch. These six were the only participants in the pipeline never told the definition of done, while `slice-tester` is asked to falsify their claims against exactly that document. Withholding the builder's reasoning from the tester is the point of context separation; withholding the shared standard from the builder was never part of it.
- **`tools:` allowlists on `slice-verifier` and `library-entry-writer`.** `docs/agents.md` advertises per-agent allowlists as platform-enforced rather than suggested, and these two shipped with none. `slice-verifier` gets `Read` / `Glob` / `Grep` / `Bash`, and omitting `Write` / `Edit` makes its documented "Read-and-run, never author" invariant structural rather than advisory. `library-entry-writer` gets `Read` / `Write` / `Edit` / `Glob` / `Grep` / `Bash` plus its three shadcn MCP tools. **Bash is required**, not optional: the agent runs `git add` on every file under the library root, and an allowlist without it would silently break staging at the exact HITL gate the change strengthens.
- **`installed_files` on `block-composer`'s output contract**, recording what `npx shadcn@latest add` actually wrote rather than what was requested. Nothing downstream could verify an install before this.
- **Two `library-gate-guard.sh` regression cases** covering `sell-pie` (warns with no approval on record, passes once one is), plus a rename of the unrelated-skill case now that two skills arm the guard. Suite is 111 tests, up from 107.

### Changed

- **`hitl_category` widened to all four categories** on `modern-ux-expert`, `layout-architect`, `block-composer`, `component-crafter`, and `state-illustrator`, which were restricted to `creative_direction` alone. A producer that found the UX spec missing had no legal category to report it under, which is the most likely thing any of them will actually encounter.
- **`ui_coverage_percent` is documented as a router, not a floor.** Below 60 with a gap matching the behavioral-interaction keyword set, `block-composer` now says in its summary that the slice is hand-rolling territory. Deliberately not a threshold: a hard floor fires on legitimately novel slices and becomes the gate operators learn to wave through.
...
- **The Library Preview Gate's hook backstop is armed under `/sell-pie`.** `hooks/library-gate-guard.sh` read `[ "$STATE_SKILL" != "sell-slice" ] && exit 0`, so the guard exited immediately in the unattended mode: the one place where autonomy makes it matter. A headline design control that was theater exactly where nobody is watching. Now a `case` over `sell-slice|sell-pie`. No precheck change was needed: `last-precheck.json` is written unconditionally and `bts_detect_skill` already returns the literal `sell-pie`, so the state was on disk today. Every fail-open property survives unchanged, including the four early exits, the session-id dedup, `BTS_HOOKS_DISABLED`, and exit 0 always.
- **Two agents no longer defer to a verifier that never loads.** `component-crafter` closed its single most important hard constraint (token-only output) with "The visual-reviewer will catch it", and `state-illustrator` did the same for `prefers-reduced-motion`. `visual-reviewer` appears zero times in `plugin.json`'s `agents[]`: it is deprecated and never loads, so both agents were relaxing their own first-pass care against a safety net that is not there. `component-crafter` now names the two live gates that do catch it. `state-illustrator` now says plainly that nothing downstream catches motion suppression today and its own verification step is the only gate, which is true as of this commit. A third dead reference in `library-entry-writer`'s waiver-rejection example was retargeted to a live agent in the same pass.
- **A standing authoring rule in `docs/architecture.md`**: no agent may name a verifier that is not in the live dispatch table. An agent that closes a constraint with "X will catch it" is trading away first-pass care for a promise, and a phantom promise makes the constraint weaker than it reads. Name a live verifier, or state that nothing downstream catches it.
