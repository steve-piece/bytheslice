...
4. **Workflow B — verify-once** (`state-illustrator → slice-tester → slice-verifier`, off-context fix loop). **Run as a `pipeline()` `Workflow`; if `Workflow` is unavailable, fall back per `loop-workflow-fallback-pattern.md`.**
   ...
   - **Exit criteria reach the producers too.** On a `frontend` slice, pass the slice's Exit criteria verbatim in every frontend producer's input contract. Withholding the builder's reasoning from the tester is the point of context separation; withholding the shared standard from the builder was never part of it.
