...
For the foundation slice types and the frontend producer pipeline that a slice may route through, `/sell-pie` defers entirely to `/sell-slice`'s routing — it does not re-implement it. One consequence is load-bearing here: on `type: frontend` the `build` row above **does not run**, and [`../sell-slice/agents/frontend/state-illustrator.md`](../sell-slice/agents/frontend/state-illustrator.md) emits the build manifest instead, at the Workflow B entry. For model-override paths, see [`../setup-shop/references/model-tier-guide.md`](../setup-shop/references/model-tier-guide.md).
...
2. **Workflow A — produce.** Dispatch the **builder** (`bytheslice:implementer`, [definition](../sell-slice/agents/implementer.md)) for each in-scope checklist item (DB-schema-first for `db-schema` / `full-stack`; `frontend` runs the producer pipeline). The builder writes the slice + its **unit tests**, makes it compile/run + smoke-pass, and **emits the build manifest** (§Appendix A). Merge per-item manifests into one **slice manifest**. Independent items may `parallel()`; dependent items `pipeline()`.
   - **On `type: frontend` no builder is dispatched**, so no manifest is produced here. The emitter is **`state-illustrator`** at step 4 below. Do not enter Workflow B expecting a manifest from this step on a frontend slice.
...
4. **Workflow B — verify-once** (`state-illustrator → slice-tester → slice-verifier`, off-context fix loop). **Run as a `pipeline()` `Workflow`; if `Workflow` is unavailable, fall back per `loop-workflow-fallback-pattern.md`.**
   - **`state-illustrator`** (frontend / full-stack with UI only) fills loading / empty / error / success states. **On `type: frontend` it is also the build-manifest emitter** (§Appendix A): pass it the slice id, the slice type, and the implemented-files list, and take `build_manifest` off its return as **the** slice manifest for the two agents below. It derives that manifest from the files on disk; `slice-verifier` re-derives its own count from the diff, independently, so this is one emission plus one independent check, not two opinions. A `frontend` slice with zero interactive surfaces still needs a manifest: dispatch with `mode: "manifest-only"`, which skips the state audit and returns the manifest alone. On `full-stack` the builder already emitted the manifest at step 2, so merge any affordance `state-illustrator` adds (retry buttons, empty-state CTAs, dismiss controls) into it before dispatching the tester, or the backstop counts them in the diff and reports an under-declaration the builder never caused. If it returns `needs_human`, or returns `complete` with an empty `build_manifest` on a frontend slice, that is a `prd_ambiguity` HITL that **halts the loop**. Never run the tester against nothing.
...
[ ] For each slice: the manifest emitter for that slice type produced a complete **build manifest** (§Appendix A): the builder on `backend` / `full-stack` / `db-schema` / `infrastructure`, `state-illustrator` on `frontend`. A `frontend` slice with an empty manifest halts the loop as `prd_ambiguity`; it never passes.
